...
func TestBuildCreateArgs_CLIArgsModifier(t *testing.T) {
	req := &ContainerRequest{
		Image: "nginx:latest",
		CLIArgsModifier: func(args []string) []string {
			// replace nginx:latest with alpine:latest
			for i, arg := range args {
				if arg == "nginx:latest" {
					args[i] = "alpine:latest"
				}
			}
			return append(args, "--debug")
		},
	}

	args, err := buildCreateArgs(req, "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	lastIdx := len(args) - 1
	if args[lastIdx] != "--debug" {
		t.Errorf("expected last arg to be '--debug', got %q", args[lastIdx])
	}

	var foundAlpine, foundNginx bool
	...
	if !foundAlpine {
		t.Error("expected 'alpine:latest' to be in args")
	}
	if foundNginx {
		t.Error("expected 'nginx:latest' to be removed from args")
	}
}
